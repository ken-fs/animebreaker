...
export function AnalyticsConsent() {
  ...
  useEffect(() => {
    if (!GA_ID) return; // no property configured — never show the banner
...
function loadAnalytics() {
  ...
  if (!GA_ID) return; // no property yet — stay silent rather than 404
  const w = window as unknown as { __abGaLoaded?: boolean; dataLayer?: unknown[] };
  if (w.__abGaLoaded) return;
  w.__abGaLoaded = true;
